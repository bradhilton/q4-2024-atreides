...
# Modifications Copyright (c) Brad Hilton 2024
...
class ChatPrediction:
    generation: CompletionMessage  # type: ignore
    decoded_tokens: Optional[List[str]] = None
    logprobs: Optional[List[List[float]]] = None
...
class Llama:
    ...
    def build(
        ...
        Args:
            ...
            device (Literal["cpu", "cuda", "mps", None], optional): Device to use for inference.
                If None, the device is set to "cuda" if available, then "mps" if available, otherwise "cpu".
                Defaults to None.
        ...
        if FAIRSCALE_AVAILABLE and device == "cuda":
        ...
        if FAIRSCALE_AVAILABLE:
            parallel_rank = get_model_parallel_rank()
        else:
            parallel_rank = 0
        ckpt_path = checkpoints[parallel_rank]
        ...
        return Llama(model, tokenizer, model_args)  # type: ignore

    def __init__(
        self, model: Transformer, tokenizer: Tokenizer, args: ModelArgs
    ) -> None:
        self.args = args
        self.model = model
    ...
    def generate(
        ...
        for cur_pos in range(min_prompt_len, total_len):
            ...
            yield TokenResult(
                token=next_token[0].item(),  # type: ignore
    ...
    def chat_completion(
        ...
        max_gen_len: Optional[int] = None,
        logprobs: bool = False,
        tool_prompt_format: ToolPromptFormat = ToolPromptFormat.json,  # type: ignore
        echo: bool = False,
    ) -> ChatPrediction:
        if (
            max_gen_len is None
    ...
    def chat_completion_raw(
        ...
        return input_tokens, output_tokens  # type: ignore
